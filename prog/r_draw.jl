...
function  output_frechet_movie( m::Morphing{N,T},
                                filename::String,
                                total_frames::Int64 = 800 ) where {N,T}
...
#function   draw_image_frame( cm::ContextMovie, P, Q, p, q )
...
function  create_demo( title::String, prefix, poly_a, poly_b,
    ...
    open( prefix * "index.html", "w" ) do fl
        ...
        if  ( f_computed_d )
            output_frechet_movie_mp4( m_d, prefix*"discrete_frechet.mp4", 400 );
...
if   num_args == 0
    poly_a,poly_b = example_10( 3, 4);
    create_demo( "Example 10", "output/10/", poly_a,poly_b );
#    create_demo_files( "output/bad_1/",
#        "data/167/trajectory/20080929001429.plt",
#        "data/144/trajectory/20090322014034.plt", true, false );
    exit( 0 );
end
